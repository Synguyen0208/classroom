...
function LessonFlashCard(props) {
  ...
  const fetchData = async () => {
    const response = await FlashCardService.getLesson(clas);
  ...
  const handleSubmit = async (data) => {
    setBackdrop(true);
    await FlashCardService.addLesson(clas, data);
    success();
  };
  const handleUpdate = async (data) => {
    const { id } = data;
    setBackdrop(true);
    await FlashCardService.updateLesson(clas, id, data);
    success();
  };
  const deleteLesson = async () => {
    setOpenDialog(false);
    setBackdrop(true);
    await FlashCardService.deleteLesson(clas, idDeleteLesson);
    success();
  };
