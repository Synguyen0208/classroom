...
function FlashCard(props) {
  ...
  const fetchData = async () => {
    const response = await FlashCardService.getFlashCard(clas, lesson);
    setData(response.data);
  };
  const handleSubmit = async (data) => {
    setBackdrop(true);
    await FlashCardService.addFlashCard(clas, lesson, data);
    success();
  };
  const success = () => {
    setBackdrop(false);
  ...
  const handleUpdate = async (data) => {
    const { id } = data;
    setBackdrop(true);
    await FlashCardService.updateFlashCard(clas, lesson, id, data);
    success();
  };
  ...
  const deleteFlashCard = async () => {
    setBackdrop(true);
    setOpenDialog(false);
    await FlashCardService.deleteFlashCard(clas, lesson, idDelete);
    success();
  };
